...
console.log('🚀 Arweave Block Explorer - v4.12: Auto-render on Image, hover outline fix, minimal UI');
...
// Render session guard to invalidate in-flight async renders when filters change
let currentRenderSession = 0;
// Camera movement speed for keyboard controls
const CAMERA_PAN_STEP = 2;
// Camera target we look at; we translate camera and target together for proper navigation
...
// (copyToClipboard removed per request)
...
function addNewBlock(blockData) {
    ...
    if (activeFilterType) {
        if (activeFilterType === 'render') {
            ...
            // Hide outline (wireframe) during render mode
            ...
            if (hasType) {
                ...
                // Keep outline visible for non-render filters
...
function setBlockHoverVisuals(block, isHovered) {
    ...
    // Subtle emissive pulse on hover when supported (e.g., MeshPhong)
    if (typeof cube.material.emissive !== 'undefined') {
        cube.material.emissive.setHex(isHovered ? 0x555555 : 0x000000);
    }

    // When in render mode, outlines are globally hidden. On hover, temporarily show outline for this block only,
    // with neon pink glow for clarity, then hide again on mouse out.
    const outline = root.children.find(child => child.isLineSegments);
    if (outline) {
        if (activeFilterType === 'render') {
            if (isHovered) {
                outline.visible = true;
                if (outline.material && outline.material.color) {
                ...
                // restore hidden state for render mode and original outline color
                outline.visible = false;
                if (outline.material && root.userData && root.userData.originalOutline) {
                    outline.material.color.set(root.userData.originalOutline);
                }
            }
        }
        // For non-render modes, we leave outline visibility as managed by filtering logic.
    }
}
...
function showBlockInfo(group) {
    ...
    // Set ViewBlock link for block height
    const vbBlock = document.getElementById('viewblock-block-link');
    if (vbBlock && h != null) {
        vbBlock.href = `https://viewblock.io/arweave/block/${h}`;
    }
    ...
    txs.forEach(tx => {
        ...
        const shortId = tx.id ? `${tx.id.substring(0, 10)}…` : '(no id)';
        const sizeText = formatBytes(tx.data_size || 0);
        txDiv.innerHTML = `
            <div class="tx-row" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                <span class="tx-id">ID: ${shortId}</span>
                <span class="tx-size" style="opacity:0.85;">Size: ${sizeText}</span>
                <div class="tx-actions" style="display:flex;gap:6px;">
                    <a class="mini-link" href="https://arweave.net/${tx.id}" target="_blank" rel="noopener" style="color:#fff;">Arweave</a>
                    <a class="mini-link" href="https://viewblock.io/arweave/tx/${tx.id}" target="_blank" rel="noopener" style="color:#fff;">ViewBlock</a>
                </div>
            </div>`;
...
function onKeyDown(e) {
    if (!camera) return;
    // Only ArrowUp and ArrowDown to move vertically in world Y
    if (e.key === 'ArrowUp') {
        e.preventDefault();
        camera.position.y += CAMERA_PAN_STEP;
        cameraTarget.y += CAMERA_PAN_STEP;
        camera.lookAt(cameraTarget);
...
function teleportToCenter() {
    ...
    cameraTarget.set(center.x, center.y, center.z + 1);
    camera.lookAt(cameraTarget);
}
...
function setIsometricView() {
    if (!camera || !monolith || monolith.children.length === 0) return;
    cameraMode = 'iso';
    // Classic isometric: 35.264° elevation and 45° yaw
    const box = new THREE.Box3().setFromObject(monolith);
    const sphere = box.getBoundingSphere(new THREE.Sphere());
    const dir = new THREE.Vector3(1, 1, 1).normalize();
    const fovRad = THREE.MathUtils.degToRad(camera.fov);
    const distance = (sphere.radius / Math.sin(fovRad / 2)) * 1.15; // add a little margin
...
function toggleLegendType(type) {
    ...
    // Invalidate any in-flight rendering whenever filters change
...
function applySceneFilter() {
    // Bump session to invalidate any in-flight async renders started previously
    currentRenderSession++;
    monolith.children.forEach(blockGroup => {
        const cube = blockGroup.children.find(child => child.isMesh);
        ...
        } else if (activeFilterType === 'tweet') {
            blockGroup.visible = !!blockGroup.userData.hasPermaButler;
            if (blockGroup.visible) {
                const style = contentTypeDataStyles.tweet;
                cube.material.color.set(style.cubeColor);
                outline.material.color.set(style.outlineColor);
            }
...
async function applyRenderMode(blockGroup) {
    // Capture session at start; if it changes before we assign, we abort
    ...
    if (imageTxs.length > 0) {
        ...
        // If filters changed during load, abort and dispose materials to avoid sticky render
        if (sessionAtStart !== currentRenderSession || activeFilterType !== 'render') {
            materials.forEach(mat => {
                if (mat && mat.uniforms && mat.uniforms.uTexture && mat.uniforms.uTexture.value) {
...
function highlightLegendSelection() {
    ...
    items.forEach(item => {
        ...
        // Add/remove X mark in the color box
...
function init() {
    ...
    // (neon grid floor removed by request)
    ...
    // Add event listeners
    // Window-level: resize + keyboard only
    window.addEventListener('resize', onWindowResize, false);
    window.addEventListener('keydown', onKeyDown, false);

    // Route pointer events to the renderer canvas only, so UI clicks don't select cubes behind
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (rotBtn) {
        rotBtn.addEventListener('click', () => {
            ...
            rotBtn.textContent = isRotating ? 'STOP ROTATION' : 'ROTATION';
        });
    ...
    if (clearBtn) clearBtn.addEventListener('click', () => toggleLegendType(null));
